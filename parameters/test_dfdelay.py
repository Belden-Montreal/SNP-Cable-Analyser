...
class TestDFDelay(TestParameter):
    ...
    def testComputeParameter(self):
        ...
        self.assertAlmostEqual(parameter[0], (np.mean(openDelay[0]) + np.mean(shortDelay[0]))/4.0)
        # check the values of the port 2
        self.assertAlmostEqual(parameter[1], (np.mean(openDelay[1]) + np.mean(shortDelay[1]))/4.0)
        # check the values of the port 3
        self.assertAlmostEqual(parameter[2], (np.mean(openDelay[2]) + np.mean(shortDelay[2]))/4.0)
        # check the values of the port 4
        self.assertAlmostEqual(parameter[3], (np.mean(openDelay[3]) + np.mean(shortDelay[3]))/4.0)
